...
```mermaid
graph LR
A[env_info 环境层] --权限隔离+环境标签鉴权--> B[app_info 应用层]
B --> C[app_cluster 集群层]
...
## 1\. 全套数据库定稿建表语句（9张表，含索引、字段、约束全整改完毕，可直接执行）
...
CREATE TABLE `app_info` (
  ...
  `app_code` varchar(64) NOT NULL COMMENT '应用唯一编码，业务隔离主键',
...
### 表4：app\_deploy\_group 部署组核心配置表（新增发布锁全套字段）
...
### 表5：cluster\_instance 主机实例表（新增分组主机双去重索引，防重复下发作业）
...
### 表6：cicd_variable 多级变量配置表（整改唯一索引）
...
### 表7：app\_deploy\_group\_release 发布基线表（新增乐观锁，解决is\_current并发脏数据）
...
### 表8：cluster\_instance\_release 单机实例发布快照表
...
### 表9：sys\_audit\_log 全局操作审计表（银行等保强制新增，全链路留痕）
