  ...
  <div>
    <el-card>
      ...
      <el-row :gutter="16" style="margin-bottom:16px">
        <el-col :span="6"><el-statistic title="应用总数" :value="stats.total" /></el-col>
        <el-col :span="6"><el-statistic title="VM应用" :value="stats.vm" /></el-col>
        <el-col :span="6"><el-statistic title="容器应用" :value="stats.container" /></el-col>
        <el-col :span="6"><el-statistic title="其他" :value="stats.other" /></el-col>
      </el-row>
      <!-- 筛选 -->
      <div style="margin-bottom:12px;display:flex;gap:12px;flex-wrap:wrap">
        ...
        <el-select v-model="filterSys" placeholder="归属系统" clearable style="width:160px" @change="fetch">
          <el-option v-for="s in sysOptions" :key="s.id" :label="s.sys_code + '-' + s.sys_name" :value="s.id" />
        </el-select>
        <el-input v-model="filterKeyword" placeholder="应用编码 / 名称" clearable style="width:200px" @change="fetch" />
      </div>
      <el-table :data="tableData" v-loading="loading" stripe>
        ...
        <el-table-column prop="server_port" label="端口" width="70" />
        <el-table-column prop="dev_owner" label="研发负责人" width="110" show-overflow-tooltip />
        <el-table-column prop="ops_owner" label="运维负责人" width="110" show-overflow-tooltip />
        <el-table-column prop="default_bk_biz_id" label="蓝鲸业务ID" width="110" />
        <el-table-column label="操作" width="260">
          <template #default="{row}">
            <el-button size="small" type="primary" link @click="$router.push('/cluster?app_code='+row.app_code)">下钻</el-button>
    ...
    <el-dialog :title="form.id?'编辑应用':'新增应用'" v-model="visible" width="750px">
      <el-form :model="form" label-width="120px">
        ...
        <el-row :gutter="16">
          ...
          <el-col :span="8"><el-form-item label="研发负责人"><el-input v-model="form.dev_owner" /></el-form-item></el-col>
          <el-col :span="8"><el-form-item label="运维负责人"><el-input v-model="form.ops_owner" /></el-form-item></el-col>
        </el-row>
        <el-row :gutter="16">
          <el-col :span="8"><el-form-item label="服务端口"><el-input v-model="form.server_port" placeholder="8080" /></el-form-item></el-col>
          <el-col :span="8"><el-form-item label="监控端口"><el-input v-model="form.management_port" placeholder="8081" /></el-form-item></el-col>
          <el-col :span="8"><el-form-item label="蓝鲸业务ID"><el-input v-model="form.default_bk_biz_id" placeholder="200" /></el-form-item></el-col>
        </el-row>
...
import { appApi, sysApi, envApi } from '../api'
...
const stats = ref({ total: 0, vm: 0, container: 0, other: 0 })
const filterEnv = ref(route.query.env_code || '')
const filterSys = ref(Number(route.query.sys_id) || '')
...
async function fetch() {
  loading.value = true
  const res = await appApi.list({ page: page.value, size, keyword: filterKeyword.value })
  // client-side filter by sys_id and env (env not in app table, skip env filter)
  let data = res.items
  if (filterSys.value) data = data.filter(i => i.sys_id === Number(filterSys.value))
  // stats from filtered data
  const vm = data.filter(i => ['springboot','nginx','tongweb','weblogic','bes','websphere','tomcat'].includes(i.app_type)).length
  const container = data.filter(i => ['tsf-service','docker'].includes(i.app_type)).length
  stats.value = { total: data.length, vm, container, other: data.length - vm - container }
...
onMounted(() => { fetchEnvs(); fetchSys(); fetch() })
</script>
